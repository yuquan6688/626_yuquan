...
From the histograms and boxplots, we can see that the distribution of RADIO (raw counts) is highly right-skewed, with most regions recording very low values and only a few showing extremely large counts. These outliers pull the distribution’s tail to the right, as clearly reflected in the boxplot. In contrast, when the variable is normalized into RADIO_PR (penetration rate), the distribution becomes more concentrated and closer to symmetric, with only a small number of high-value outliers remaining. This indicates that using penetration rates helps to reduce scale effects caused by population size and improves comparability across regions, making it a more suitable measure for subsequent analysis.
...
the left map emphasizes population scale, while the right map emphasizes relative adoption. This contrast reveals that high household numbers do not always translate into high penetration rates, and smaller townships can sometimes achieve disproportionately higher radio ownership relative to their size.
...
Standardisation methods do not alter the underlying distributional shape of the data; instead, they change the scale and location of the values. Min–Max rescaling compresses the data into a fixed 0–1 range, while Z-score transformation centers and scales it around the mean. This ensures comparability across variables without affecting the relative pattern of variation.
...
In this section, I will learn how to derive spatially constrained cluster by using [*skater()*](https://r-spatial.github.io/spdep/reference/skater.html) method of [**spdep**](https://r-spatial.github.io/spdep/index.html)package.
